...
The streaming workflow is the same pipeline, expressed as incremental state updates instead of a single batch run.
...
The same workflow is done completely in python with the healpyxel API in the
[full workflow tutorial](notebooks/02_full_workflow.py).
...
Minimal code, a more detailed explanation is in the
[full workflow tutorial](notebooks/02_full_workflow.py).
...
The tutorial notebooks and workflow examples in this repository are intentionally grounded in MESSENGER/MASCS data products, but the same pattern applies to other sparse spherical datasets.
