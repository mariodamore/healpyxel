...
### Implemented
- **fix**: Normalize `lon_convention='minus_plus180'` → `'-180_180'` across metadata pipeline (ADR-009)
  - `metadata.py:__post_init__()` now converts on construction
  - `geospatial._extract_healpix_params_from_metadata()` now handles the alias
  - Prevents silent coordinate mismatch when regenerating geo.parquet from existing aggregates
  - QGIS visualization note: parquet spatial metadata caching may require re-adding layers or using GeoTIFF
- **feat**: Migrate from nbdev to pure Python + Quarto (ADR-007)
  - `healpyxel/*.py` files are now hand-edited source of truth (no auto-generation)
  - Removed nbdev scaffolding: cell markers, AUTOGENERATED headers, `_modidx.py`
  - Converted tutorial notebooks (`nbs/80-83_*.ipynb`, `90_*.ipynb`) to `notebooks/*.py:percent`
  - Deleted `nbs/`, `settings.ini`, `_scripts_original/`, `healpyxel/tests/` (auto-generated)
  - Removed nbdev from pyproject.toml dependencies
  - Updated docs to use Quarto + Jupyter engine
  - API reference will use quartodoc (replaces nbdev showdoc)
  - All tests consolidated to `tests/test_*.py`
