...
![Pipeline_End-to-End](diagrams/svg/Pipeline_End-to-End.svg){ height=500px }
...
`healpyxel.workflow.run_pipeline()` wraps the three CLI stages into a
single Python call and (optionally) prints/logs the equivalent CLI
commands. It uses `.meta.json` files to avoid redundant work and
auto-discovers numeric columns via `pyarrow`.
...
See [API reference → workflow.run_pipeline](reference/workflow.run_pipeline.qmd)
