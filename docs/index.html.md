# HEALPix-based Spatial Aggregation
...
- **The “Sidecar” Workflow:** If you want to index your data without
  modifying the original source files. healpyxel creates lightweight
  “sidecar” files that map your GeoParquet rows to HEALPix cells.
- **Large-Scale Data Engineering:** If your dataset is too large for
  memory. By using a **Split-Apply-Combine** approach on GeoParquet, you
  can perform global aggregations, filtering, and map generation across
  TB-scale datasets.
- **Streaming & Incremental Ingestion:** If you need to update global
  maps as new data arrives (e.g., ongoing missions or sky surveys)
  without reprocessing the entire historical archive.
- **Decoupled Responsibilities:** If you need to separate the *spatial
  indexing* (where is the data?) from the *physical aggregation* (what
  is the mean/min/max value?).

**Who is this NOT for?**

While healpyxel is powerful, it is not a “one-size-fits-all” tool for
every HEALPix task. You might consider alternatives if your use case
falls into these categories:

- **High-Resolution 2D Imagery:** If your primary data source is dense
  2D images (e.g., CCD frames), converting every single pixel into a row
  in a Parquet file is inefficient. For dense image-to-HEALPix
  re-projection, tools like
  [reproject](https://reproject.readthedocs.io/) or
  [astropy-healpix](https://astropy-healpix.readthedocs.io/) are more
  suitable.
- **Standard Xarray/Dask Unstructured Grids:** If you require deep
  integration with the Xarray ecosystem and need to work with general
  unstructured meshes beyond just HEALPix,
  [UXarray](https://uxarray.readthedocs.io/) is a better fit.
- **Multi-order Coverage (MOC) & LIGO workflows:** If you need to handle
  Multi-order HEALPix maps or specific astronomical IO formats used in
  gravitational wave science, check out
  [mhealpy](https://mhealpy.readthedocs.io/).
- **Simple Visualization:** If you only need to plot a small array of
  values on a sphere once, the overhead of the GeoParquet/Sidecar
  workflow might be unnecessary.
...
1.  **Split (The Sidecar):** Instead of rewriting your heavy raw data,
    healpyxel generates a small Parquet file containing only the index
    of the original data and its corresponding healpix_id. This is your
    “Spatial Index.”
2.  **Apply (Aggregation):** You can then join this sidecar with any
    column in your original dataset to calculate statistics (Mean, Std
    Dev, Count) per cell.
3.  **Combine (The Map):** The results are combined into a final HEALPix
    map or a streaming accumulator.

This decoupling allows you to experiment with different HEALPix
resolutions (NSIDE) or coordinate systems by only recreating the tiny
sidecar file, leaving your multi-gigabyte raw data untouched.
...
optimized to ingest these “shredded” lines at high speed to produce
seamless global mosaics.
