...
``` python
<VSCode.Cell id="#VSC-a3224b38" language="python">
#| export
#| eval: false
#| hide
...
Examples:
  ...
  # Aggregate using a specific sidecar
    ...
    if args.aggregate:
        ...
        # Load sidecar metadata JSON if available
        sidecar_metadata_json = {}
        sidecar_metadata_path = sidecar_path.with_suffix('.metadata.json')
        if sidecar_metadata_path.exists():
            logger.info(f"Loading sidecar metadata from {sidecar_metadata_path}")
            with open(sidecar_metadata_path, 'r') as f:
                sidecar_metadata_json = json.load(f)
        else:
            logger.warning(f"No sidecar metadata JSON found at {sidecar_metadata_path}")
        ...
        # Add metadata to output - merge sidecar metadata for complete provenance
        sidecar_metadata = sidecars_df.iloc[args.sidecar_index].to_dict()
        
        # Build aggregation metadata with timestamp
        ...
        output_metadata = {
            'processing': {
                ...
                'output_file': str(args.output.absolute())
            },
            'aggregation': {
                'value_columns': args.columns,
            ...
            'sidecar_metadata': sidecar_metadata_json  # Include complete sidecar provenance
        }
        
        # Also preserve legacy flat metadata from parquet schema for backwards compatibility
